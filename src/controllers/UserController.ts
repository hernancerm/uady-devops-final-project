...
export const UserController = (userRepository: Repository<User>) => {
  ...
  const getToken = async (req: Request, res: Response): Promise<Response> => {
    const providedUser = Object.assign(new User(), req.body);
    const user = await userRepository.findOne({where: {username: providedUser.username}});
    if(!user) {
      return res.status(401).json({ error: "Invalid username or password" });
    }
    return await new Promise<Response>(function(resolve) {
      bcrypt.compare(providedUser.password, user.password, function(err, reslt) {
        if(err || !reslt) {
          resolve(res.status(401).json({ error: "Invalid username or password" }));
        } else {
          resolve(res.status(200).json({"token": AuthHelper().getToken(user.username)}));
        }
      });
    });
  };
